<div>
    <div class="bg-white mx-auto px-4 sm:px-6 lg:px-8 xl:px-[10%] py-32 bg-cover bg-center"
        ...
        <div class="mb-10">
            ...
            <h1 class="text-2xl md:text-3xl font-bold text-slate-900">
                Buat Caption & Cerita Produk dalam Sekejap
            </h1>
            <p class="text-sm md:text-base text-slate-600 mt-2 max-w-2xl">
                Unggah foto produk Anda, biarkan AI mengenali motifnya, menyusun cerita budaya yang menarik, 
                dan membuatkan caption promosi siap posting. Hemat waktu, tingkatkan nilai jual.
            </p>
        </div>
        ...
        <form wire:submit.prevent="analyze"
            ...
            <div
                ...
                <div class="pt-5 mt-5 border-t border-dashed border-amber-100">
                    ...
                    <div class="space-y-4 rounded-2xl bg-amber-50/40 px-3 py-4">
                        {{-- Segmented buttons --}}
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-3 gap-y-4 text-[11px] md:text-xs">
                            {{-- Indonesia --}}
                            ...
                            <button type="button" wire:click="$set('captionLanguage', 'santai')"
                                ...
                                , 'border-amber-400 bg-amber-50 text-amber-900 shadow-sm'=> $captionLanguage === 'santai',
                                'border-slate-200 text-slate-600 hover:border-amber-300 hover:bg-amber-50/60' =>
                        ...
                        <p class="text-[10px] text-slate-500 leading-relaxed">
                            @if ($captionLanguage === 'id')
                            AI akan menyusun caption dalam Bahasa Indonesia dengan gaya formal dan promosi yang profesional
                            untuk UMKM.
                            @elseif ($captionLanguage === 'santai')
                            AI akan menyusun caption dengan gaya santai, casual, dan friendly yang cocok untuk media sosial dan lebih dekat dengan audiens muda.
            ...
            <div
                ...
                <div wire:loading wire:target="analyze,publish"
                    class="w-full flex flex-col items-center justify-center text-center gap-4 py-12">
                    
                    <div class="relative w-16 h-16 mx-auto">
                        {{-- Outer Ring --}}
                        <div class="absolute inset-0 border-4 border-amber-100 rounded-full"></div>
                        {{-- Spinning Segment --}}
                        <div class="absolute inset-0 border-4 border-amber-600 border-t-transparent rounded-full animate-spin"></div>
                        {{-- Inner Icon --}}
                        <div class="absolute inset-0 flex items-center justify-center">
                            <svg class="w-6 h-6 text-amber-700 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="space-y-1 mx-auto">
                        <p class="text-slate-900 font-semibold text-lg animate-pulse">
                            Meracik Cerita...
                        </p>
                        <p class="text-xs text-slate-500 max-w-xs mx-auto leading-relaxed">
                            AI sedang menganalisis motif dan menyusun narasi budaya yang autentik untuk Anda.
                        </p>
                    </div>
                </div>
                ...
                <div wire:loading.remove wire:target="analyze,publish" class="space-y-5">
                    ...
                    <div class="flex items-start justify-between gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            @if ($story->poster_path)
                            <div class="text-center">
                                <p class="text-[10px] text-slate-400 mb-1">Foto produk</p>
                                <img src="{{ asset('storage/' . $story->poster_path) }}"
                            ...
                            <div class="text-center group relative">
                                ...
                                <div class="relative">
                                    ...
                                    <button onclick="downloadQr('{{ asset('storage/' . $story->qr_code_path) }}', '{{ $story->detected_motif }}-QR.png')"
                                        ...
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4-4m0 0L8 8m4-4v12"></path></svg>
                                    </button>
                                </div>
                            </div>
                            @endif
                    ...
                    <div class="flex items-start justify-between gap-4">
                        ...
                        <div class="text-center shrink-0">
                            ...
                            <div class="relative group">
                                <img src="{{ Storage::url($story->image_path) }}"
                                    class="w-20 h-20 object-cover rounded-xl border border-slate-200" alt="Product Image">
                                
                                @if($story->poster_path)
                                <button type="button" wire:click="$set('showPosterModal', true)" 
                                        class="absolute inset-0 bg-black/40 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-1">
                                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    <span class="text-[8px] text-white font-medium">Lihat Poster</span>
                                </button>
                                @endif
                            </div>
                        </div>
                        @elseif ($posterUrl)
                        <div class="text-center shrink-0">
                            <p class="text-[10px] text-slate-400 mb-1">Preview Poster</p>
                            <button type="button" wire:click="$set('showPosterModal', true)" class="relative group">
                                <img src="{{ $posterUrl }}"
                                    class="w-20 h-20 object-cover rounded-xl border border-slate-200 transition-transform group-hover:scale-105" alt="Generated Poster">
                        ...
                        @elseif ($isGeneratingPoster)
                        <div class="text-center shrink-0" wire:init="generatePoster">
                            <p class="text-[10px] text-slate-400 mb-1">Mendesain...</p>
                            <div class="w-20 h-20 rounded-xl border border-slate-200 bg-slate-50 flex flex-col items-center justify-center gap-2 animate-pulse">
                    ...
                    <div class="flex justify-end pt-2">
                        <button type="button" wire:click="publish" 
                                @if($isGeneratingPoster) disabled @endif
    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center px-4 sm:px-6"
        ...
        {{-- Modal Content --}}
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 md:p-8 text-center transform transition-all scale-100">
            ...
            <h3 class="text-xl font-bold text-slate-900 mb-2">Motif Sakral Terdeteksi tidak bisa di perjual belikan</h3>
    ...
    @if($showPosterModal && ($posterUrl || ($story && $story->poster_path)))
    <div class="fixed inset-0 z-[100] flex items-center justify-center px-4 sm:px-6"
         x-data
         x-init="$nextTick(() => { document.body.classList.add('overflow-hidden'); });"
         @keydown.escape.window="document.body.classList.remove('overflow-hidden'); $wire.set('showPosterModal', false)">
        
        {{-- Backdrop --}}
        <div class="absolute inset-0 bg-slate-900/90 backdrop-blur-sm transition-opacity" 
             wire:click="$set('showPosterModal', false)"
             @click="document.body.classList.remove('overflow-hidden')"></div>

        {{-- Modal Content --}}
        <div class="relative w-full max-w-lg transform transition-all scale-100">
            {{-- Close Button --}}
            <button wire:click="$set('showPosterModal', false)" 
                    @click="document.body.classList.remove('overflow-hidden')"
            ...
            <div class="bg-white rounded-2xl overflow-hidden shadow-2xl">
                @php
                    $modalImage = $story && $story->poster_path ? Storage::url($story->poster_path) : $posterUrl;
                @endphp
                <img src="{{ $modalImage }}" alt="Full Poster" class="w-full h-auto">
                
                <div class="p-4 bg-white flex items-center justify-between">
                    <p class="text-sm font-medium text-slate-900">Poster AI</p>
                    <a href="{{ $modalImage }}" download="poster-carita.jpg" target="_blank" 
